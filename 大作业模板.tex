...
\subsection{数据库表清单}
根据当前实际数据库 \texttt{pickpass\_db} 的在线结构，系统现有 9 张核心业务表，具体如下：
...
各表之间的核心关系为：\texttt{sys\_user} 与 \texttt{base\_info} 通过负责人字段关联，\texttt{base\_info} 与 \texttt{recruitment\_job} 构成“一基地对多岗位”，\texttt{daily\_signup} 连接用户、基地与岗位，\texttt{labor\_salary} 基于签到记录生成，\texttt{salary\_payment} 再对工资记录进行支付确认，从而形成“用户—岗位—签到—结算—发放”的完整业务链路。
...
\field{role_key} & enum & 非空，默认 \field{worker} & 用户角色，如工人、基地管理员、超级管理员。 \\
...
\field{phone_hash} & varchar(64) & 非空，唯一索引 & 手机号 Hash，用于登录检索与去重。 \\
...
\field{paymentMethod} & enum & 非空 & 支付方式：现金或转账。 \\
\field{status} & tinyint & 非空，默认 0 & 支付状态。 \\
...
\field{operationType} & enum & 非空 & 操作类型，如创建、修改、审核、支付等。 \\
\field{resourceType} & enum & 非空 & 资源类型，如用户、基地、岗位、签到、工资。 \\
