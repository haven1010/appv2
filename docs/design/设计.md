...
**实现文件**：
- `apps/backend/src/modules/user/entities/sys-user.entity.ts` - 用户实体（含紧急联系人字段）
- `apps/backend/src/modules/user/user.controller.ts` - 用户控制器（OCR注册、信息更新）
- `apps/backend/src/modules/user/user.service.ts` - 用户服务（信息更新、审核）
- `apps/backend/src/modules/common/services/backup.service.ts` - 数据备份服务
...
**实现文件**：
- `apps/backend/src/modules/user/entities/sys-user.entity.ts` - 用户角色枚举
- `apps/backend/src/modules/auth/guards/roles.guard.ts` - 角色权限守卫
- `apps/backend/src/modules/common/entities/operation-log.entity.ts` - 操作日志实体
- `apps/backend/src/modules/common/services/operation-log.service.ts` - 操作日志服务
...
**实现文件**：
- `apps/backend/src/modules/attendance/attendance.controller.ts` - 考勤控制器
- `apps/backend/src/modules/attendance/attendance.service.ts` - 考勤服务（含代报名、短信发送）
- `apps/backend/src/modules/common/services/sms.service.ts` - 短信服务
...
**实现文件**：
- `apps/backend/src/modules/salary/salary.service.ts` - 工资计算服务
- `apps/backend/src/modules/salary/services/salary-calculator.strategy.ts` - 工资计算策略
- `apps/backend/src/modules/salary/entities/salary-payment.entity.ts` - 工资支付实体
- `apps/backend/src/modules/salary/services/salary-payment.service.ts` - 工资支付服务
...
**实现文件**：
- `apps/backend/src/modules/base/base.controller.ts` - 基地控制器
- `apps/backend/src/modules/base/base.service.ts` - 基地服务
- `apps/backend/src/modules/base/entities/base-cooperation.entity.ts` - 基地合作申请实体
- `apps/backend/src/modules/base/entities/job-application.entity.ts` - 岗位申请实体
- `apps/backend/src/modules/base/services/job-application.service.ts` - 岗位申请服务
- `apps/backend/src/modules/base/services/base-cooperation.service.ts` - 基地合作服务
- `apps/backend/src/modules/recommendation/recommendation.service.ts` - 智能推荐服务
...
**实现文件**：
- `apps/backend/src/modules/base/services/map.service.ts` - 地图服务（预留）
- `apps/backend/src/modules/base/entities/base-rating.entity.ts` - 基地评价实体（预留）
...
## 九、Design System（v1.0）补充

已新增可落地的设计系统文档，路径如下：

- `apps/web-admin/docs/picker-design-system-v1.md`
- `apps/web-admin/docs/login-role-split-crayon-ui.md`

本版设计系统已统一蓝黄视觉语言、核心组件规范、页面模式与作品集表达模板，可直接用于前端实现与展示材料整理。
新增的 Crayon 登录分流文档覆盖了角色识别、双端登录、企业审核、二维码签到与成长反馈，可直接用于作品集展示与需求评审。
