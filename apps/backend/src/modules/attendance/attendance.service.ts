...
import { RecruitmentJob, JobAuditStatus, JobStatus } from '../base/entities/recruitment-job.entity';
import { ApplicationStatus } from '../base/entities/job-application.entity';
...
export class AttendanceService {
  ...
  private parseWorkHoursRange(workHours?: string | null): { start: number; end: number } | null {
  ...
  private hasWorkHoursOverlap(leftWorkHours?: string | null, rightWorkHours?: string | null): boolean {
    const left = this.parseWorkHoursRange(leftWorkHours);
    const right = this.parseWorkHoursRange(rightWorkHours);
    if (!left || !right) return true;
    return left.start < right.end && right.start < left.end;
  }

  private hasTimeOverlap(leftWorkHours?: string | null, rightWorkHours?: string | null): boolean {
    return this.hasWorkHoursOverlap(leftWorkHours, rightWorkHours);
  }

  private isWorkDateWithinJobWindow(job: any, workDate: string): boolean {
    const target = this.normalizeDateString(workDate);
    if (!target) return false;

    const start = this.normalizeDateString(job?.workStartDate ?? job?.work_start_date);
    const end = this.normalizeDateString(job?.workEndDate ?? job?.work_end_date);
    if (start && target < start) return false;
    if (end && target > end) return false;
    return true;
  }

  private buildSignupConflictException(conflictRow: any): ConflictException {
    const baseName = String(conflictRow?.baseName || conflictRow?.base_name || '未知基地');
    const jobTitle = String(conflictRow?.jobTitle || conflictRow?.job_title || '未知岗位');
    const message = `您已报名【${baseName} / ${jobTitle}】，时间冲突。如需报名此工作，请先取消原报名。`;
    return new ConflictException({
      message,
      conflictBaseId: Number(conflictRow?.baseId ?? conflictRow?.base_id ?? 0),
      conflictBaseName: baseName,
      conflictJobId: Number(conflictRow?.jobId ?? conflictRow?.job_id ?? 0),
      conflictJobTitle: jobTitle,
      conflictWorkDate: this.normalizeDateString(conflictRow?.workDate ?? conflictRow?.work_date),
      conflictWorkHours: String(conflictRow?.workHours ?? conflictRow?.work_hours ?? ''),
    });
  }

  private assertJobRowOpenForSignup(job: any, expectedBaseId: number, workDate: string): void {
    if (!job) {
      throw new NotFoundException('该岗位不存在');
    }

    if (Number(job.baseId ?? job.base_id) !== Number(expectedBaseId)) {
      throw new BadRequestException('岗位与基地不匹配');
    }

    if (Number(job.baseAuditStatus ?? job.base_audit_status ?? JobAuditStatus.APPROVED) !== Number(JobAuditStatus.APPROVED)) {
      throw new BadRequestException('基地审核通过后才能报名');
    }

    if (!job.isActive && Number(job.is_active) !== 1) {
      throw new BadRequestException('该岗位尚未开放报名');
    }

    if (Number(job.status) !== Number(JobStatus.RECRUITING)) {
      throw new BadRequestException('该岗位尚未开放报名');
    }

    const validUntil = job.validUntil ?? job.valid_until;
    if (validUntil && new Date(validUntil).getTime() <= Date.now()) {
      throw new BadRequestException('该岗位已过期');
    }

    if (!this.isWorkDateWithinJobWindow(job, workDate)) {
      throw new BadRequestException('报名日期不在该岗位工作日期范围内');
    }
  }

  private async getJobRowForSignup(manager: any, jobId: number): Promise<any> {
    const rows = await manager.query(
      `SELECT
         job.id,
         job.base_id AS baseId,
         job.job_title AS jobTitle,
         job.work_hours AS workHours,
         job.work_start_date AS workStartDate,
         job.work_end_date AS workEndDate,
         job.status,
         job.is_active AS isActive,
         job.valid_until AS validUntil,
         base.base_name AS baseName,
         base.audit_status AS baseAuditStatus
  ...
  private async refreshApplicantCountInManager(manager: any, jobId: number): Promise<number> {
    const rows = await manager.query(
      `SELECT COUNT(*) AS applicantCount
         FROM job_application
        WHERE job_id = ? AND status IN (?, ?)`,
      [jobId, ApplicationStatus.PENDING, ApplicationStatus.APPROVED],
    );
    const applicantCount = Number(rows?.[0]?.applicantCount ?? rows?.[0]?.applicant_count ?? 0);
    await manager.query(
  ...
  private async ensureApplicationForSignupInManager(
    manager: any,
    userId: number,
    jobId: number,
    baseId: number,
    ...
    if (existing && [ApplicationStatus.PENDING, ApplicationStatus.APPROVED].includes(Number(existing.status))) {
      return {
    ...
    if (existing && Number(existing.status) === ApplicationStatus.CANCELLED) {
      await manager.query(
        `UPDATE job_application
            SET status = ?, note = ?, reject_reason = NULL, reviewed_by = NULL, reviewed_at = NULL, updated_at = NOW()
          WHERE id = ?`,
        [ApplicationStatus.PENDING, note?.trim() || '', existing.id],
      );
      return {
        id: Number(existing.id),
    ...
    const result = await manager.query(
      `INSERT INTO job_application (user_id, job_id, base_id, status, note, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, NOW(), NOW())`,
      [userId, jobId, baseId, ApplicationStatus.PENDING, note?.trim() || ''],
    );
   ...
   */
  private async signupTransactional(userId: number, dto: any, context?: OperationLogContext): Promise<any> {
    const baseId = Number(dto?.baseId || 0);
    const jobId = Number(dto?.jobId || 0);
    const workDate = this.normalizeDateString(dto?.workDate) || this.getTodayDateString();
    ...
    if (!baseId || !jobId) {
      throw new BadRequestException('请选择基地和岗位');
    }
    ...
    const result = await this.dataSource.transaction(async (manager) => {
      const signupRepo = manager.getRepository(DailySignup);
      const job = await this.getJobRowForSignup(manager, jobId);
      this.assertJobRowOpenForSignup(job, baseId, workDate);

      const sameJobRows = await manager.query(
        `SELECT id, user_id AS userId, base_id AS baseId, job_id AS jobId, work_date AS workDate, status
           FROM daily_signup
          WHERE user_id = ? AND base_id = ? AND job_id = ? AND work_date = ?
            AND status IN (?, ?)
          ORDER BY id DESC
          LIMIT 1
          FOR UPDATE`,
        [userId, baseId, jobId, workDate, SignupStatus.SIGNED_UP, SignupStatus.CHECKED_IN],
      );

      if (sameJobRows?.[0]) {
        const existingSignup = sameJobRows[0];
        const application = await this.ensureApplicationForSignupInManager(manager, userId, jobId, baseId, note);
        await this.refreshApplicantCountInManager(manager, jobId);
        return {
          signup: {
            id: Number(existingSignup.id),
            userId: Number(existingSignup.userId),
            baseId: Number(existingSignup.baseId),
            jobId: Number(existingSignup.jobId),
            workDate,
            status: Number(existingSignup.status),
            duplicate: true,
            applicationId: application.id,
          },
          proxySignups: [] as DailySignup[],
          duplicate: true,
        };
      }

      const conflictRows = await manager.query(
        `SELECT signup.id, signup.base_id AS baseId, signup.job_id AS jobId, signup.work_date AS workDate, signup.status,
                base.base_name AS baseName, job.job_title AS jobTitle, job.work_hours AS workHours
          ...
          FOR UPDATE`,
        [userId, workDate, SignupStatus.SIGNED_UP, SignupStatus.CHECKED_IN],
      );

      const conflictingSignup = (conflictRows || []).find((row: any) => this.hasWorkHoursOverlap(job.workHours, row.workHours));
      if (conflictingSignup) {
        throw this.buildSignupConflictException(conflictingSignup);
      }
      ...
      const savedSignup = await signupRepo.save(signup);

      const savedProxySignups: DailySignup[] = [];
      for (const proxyUserId of proxyUserIds) {
        ...
        try {
          savedProxySignups.push(await signupRepo.save(proxySignup));
        } catch (error) {
          this.rethrowDuplicateSignup(error, `用户 ${proxyUserId} 当日已报名该基地`);
        }
      }
      ...
      return {
        ...
        proxySignups: savedProxySignups,
        duplicate: false,
      };
    });
    ...
    await this.operationLogService.logWithContext({
      operationType: OperationType.CREATE,
      resourceType: ResourceType.SIGNUP,
      resourceId: result.signup.id,
      userId,
      request: context?.request,
      description: `创建报名记录: signupId=${result.signup.id}, baseId=${baseId}, jobId=${jobId}, duplicate=${result.duplicate ? 1 : 0}`,
      afterData: {
        baseId,
        jobId,
        workDate,
        ...
        applicationId: result.signup.applicationId,
      },
    });
    ...
    if (!result.duplicate) {
      try {
        ...
        this.logger.warn(`发送报名短信失败: ${error.message}`);
      }
    }

    for (const savedProxySignup of result.proxySignups) {
      await this.operationLogService.logWithContext({
        operationType: OperationType.CREATE,
        resourceType: ResourceType.SIGNUP,
  ...
  async signup(userId: number, dto: any, context?: OperationLogContext): Promise<DailySignup> {
    return this.signupTransactional(userId, dto, context);
    ...
    if (!normalizedBaseId || !normalizedJobId) {
      throw new BadRequestException('请选择基地和岗位');
    }
    ...
    const signupResult = await this.dataSource.transaction(async (manager) => {
      const jobRepo = manager.getRepository(RecruitmentJob);
      const signupRepo = manager.getRepository(DailySignup);

      const job = await jobRepo.findOne({ where: { id: normalizedJobId, baseId: normalizedBaseId } });
      if (!job) {
      ...
      if (Number(job.auditStatus) !== Number(JobAuditStatus.APPROVED)) {
        throw new BadRequestException('璇ュ矖浣嶅皻鏈鏍搁€氳繃锛屾殏涓嶅彲鎶ュ悕');
      }
      if (Number(job.status) !== Number(JobStatus.RECRUITING)) {
        throw new BadRequestException('璇ュ矖浣嶅綋鍓嶄笉鍦ㄦ嫑鑱樹腑锛屾殏涓嶅彲鎶ュ悕');
      }
      if (job.validUntil && new Date(job.validUntil).getTime() <= Date.now()) {
        throw new BadRequestException('璇ュ矖浣嶅凡杩囨湡锛屾殏涓嶅彲鎶ュ悕');
      }

      const sameJobRows = await manager.query(
        `SELECT id, user_id AS userId, base_id AS baseId, job_id AS jobId, work_date AS workDate,
                status, checkin_time AS checkinTime, is_proxy AS isProxy, proxy_user_id AS proxyUserId,
                is_offline_sync AS isOfflineSync, created_at AS createdAt, updated_at AS updatedAt
           FROM daily_signup
          WHERE user_id = ? AND base_id = ? AND job_id = ? AND work_date = ?
            AND status IN (?, ?)
          ORDER BY id DESC
          LIMIT 1
          FOR UPDATE`,
        [userId, normalizedBaseId, normalizedJobId, workDate, SignupStatus.SIGNED_UP, SignupStatus.CHECKED_IN],
      );
      if (sameJobRows[0]) {
        return {
          savedSignup: signupRepo.create(sameJobRows[0] as Partial<DailySignup>) as DailySignup,
          savedProxySignups: [] as DailySignup[],
          isDuplicate: true,
        };
      }

      const existingRows = await manager.query(
        `SELECT signup.id, signup.base_id AS baseId, signup.job_id AS jobId, signup.work_date AS workDate,
                base.base_name AS baseName, job.job_title AS jobTitle, job.work_hours AS workHours
           FROM daily_signup signup
           LEFT JOIN base_info base ON base.id = signup.base_id
           LEFT JOIN recruitment_job job ON job.id = signup.job_id
      ...
      const conflict = existingRows.find((row: any) => this.hasTimeOverlap(job.workHours, row.workHours));
      if (conflict) {
        throw this.buildSignupConflictException(conflict);
      }
      ...
      try {
        ...
        if (error?.code === 'ER_DUP_ENTRY') {
          ...
          if (duplicateRows[0]) {
            return {
              savedSignup: signupRepo.create(duplicateRows[0] as Partial<DailySignup>) as DailySignup,
              savedProxySignups: [] as DailySignup[],
              isDuplicate: true,
            };
          }
        }
        this.rethrowDuplicateSignup(error, '您今日已在该基地报名，请勿重复操作');
      }
    ...
    const { savedSignup, savedProxySignups, isDuplicate } = signupResult;
    if (isDuplicate) {
      return Object.assign(savedSignup, { duplicate: true }) as DailySignup;
    }
    ...
    try {
      const qrCode = await this.generateUserQrCode(userId);
      const base = await this.userRepo.manager.getRepository('BaseInfo').findOne({ where: { id: normalizedBaseId } });
      const user = await this.userRepo.findOne({ where: { id: userId } });
    ...
    return Object.assign(savedSignup, { duplicate: false }) as DailySignup;
  }
   ...
   */
  private async cancelSignupTransactional(
    userId: number,
    dto: { signupId?: number; baseId?: number; jobId?: number; workDate?: string },
    context?: OperationLogContext,
  ) {
    ...
    const deletedSignup = await this.dataSource.transaction(async (manager) => {
      ...
      const where: any = signupId
        ? { id: signupId, userId }
        : (jobId ? { userId, baseId, jobId, workDate } : { userId, baseId, workDate });
      ...
      const applicationRows = await manager.query(
        `SELECT id, status
           FROM job_application
          ...
          LIMIT 1
          FOR UPDATE`,
        [userId, signup.jobId, signup.baseId, ApplicationStatus.PENDING, ApplicationStatus.APPROVED],
      );
      ...
      if (applicationRows?.[0]) {
        await manager.query(
          `UPDATE job_application
            ...
            WHERE id = ?`,
          [ApplicationStatus.CANCELLED, applicationRows[0].id],
        );
        applicationCancelled = true;
      }

      await this.refreshApplicantCountInManager(manager, signup.jobId);
      return {
        signup,
    ...
    await this.operationLogService.logWithContext({
      ...
      resourceType: ResourceType.SIGNUP,
      resourceId: deletedSignup.signup.id,
      userId,
      request: context?.request,
      description: `取消报名并删除记录: signupId=${deletedSignup.signup.id}, baseId=${deletedSignup.signup.baseId}, jobId=${deletedSignup.signup.jobId}`,
      beforeData: {
        id: deletedSignup.signup.id,
        userId: deletedSignup.signup.userId,
        baseId: deletedSignup.signup.baseId,
        jobId: deletedSignup.signup.jobId,
        workDate: deletedSignup.signup.workDate,
        status: deletedSignup.signup.status,
        isProxy: deletedSignup.signup.isProxy,
      },
      afterData: {
        deleted: true,
        applicationCancelled: deletedSignup.applicationCancelled,
      },
    });
    ...
    return {
      success: true,
      deleted: true,
      signupId: deletedSignup.signup.id,
      applicationCancelled: deletedSignup.applicationCancelled,
      baseId: deletedSignup.signup.baseId,
      jobId: deletedSignup.signup.jobId,
      workDate: deletedSignup.signup.workDate,
  ...
  async cancelSignup(
    ...
    return this.cancelSignupTransactional(userId, dto, context);
   ...
   */
  private async getWorkerSignupRecordsNormalized(userId: number, limit = 50) {
    ...
    return list.map((r) => ({
      ...
      canCancel: r.status === SignupStatus.SIGNED_UP ? 1 : 0,
    }));
  }

  async getWorkerSignupRecords(userId: number, limit = 50) {
    return this.getWorkerSignupRecordsNormalized(userId, limit);
