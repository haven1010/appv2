...
export class AuthController {
  ...
  @Post('wechat-login')
  @ApiOperation({ summary: '微信一键登录', description: '使用 wx.login 获取的 code 登录。' })
  async wechatLogin(@Body('code') code: string, @Req() req) {
    if (!code) {
      throw new UnauthorizedException('缺少微信登录凭证 code');
    }
    const result = await this.authService.wechatLogin(code, req);
    return {
