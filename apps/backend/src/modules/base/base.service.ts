...
export class BaseService {
  ...
  async findAll(query: any): Promise<BaseInfo[]> {
    ...
    const hasOpenJob = query?.hasOpenJob === true
      || query?.hasOpenJob === 'true'
      || query?.hasOpenJob === '1'
      || query?.hasOpenJob === 1;
    ...
    if (query.ownerId) {
      ...
      const rawUsers = await this.dataSource.query(
        `SELECT id, role_key AS roleKey
           FROM sys_user
          WHERE id = ? AND is_deleted = 0
          LIMIT 1`,
        [ownerId],
      );
      const targetUser = rawUsers?.[0] || null;
      if (!targetUser) {
        return [];
      }

      if (targetUser.roleKey === UserRole.BOSS) {
        qb.andWhere('base.ownerId = :ownerId', { ownerId: Number(targetUser.id) });
    ...
    if (hasOpenJob) {
      qb.andWhere(
        `EXISTS (
          ...
          WHERE job.base_id = base.id
            AND job.is_active = :jobIsActive
            AND job.audit_status = :jobAuditStatus
            AND job.status = :jobStatus
            AND (job.valid_until IS NULL OR job.valid_until > :now)
        )`,
        {
          jobIsActive: true,
          jobAuditStatus: JobAuditStatus.APPROVED,
  ...
  async remove(id: number, operatorId: number, context?: OperationLogContext): Promise<{ msg: string }> {
    ...
    try {
      purgeResult = await this.dataSource.transaction(async (manager) => {
        const base = await manager.findOne(BaseInfo, {
          where: { id },
          lock: { mode: 'pessimistic_write' },
        });
      ...
      this.logger.error(`[删除基地] 失败: baseId=${id}, reason=${reason}`);
      if (
        /field_manager must have assigned_base_id/i.test(reason)
        || /only field_manager can set assigned_base_id/i.test(reason)
      ) {
        throw new ConflictException('基地管理员绑定状态异常，请先在用户管理中修正该基地关联后再删除');
      }
      if (/base owner must be base_manager or boss/i.test(reason)) {
        throw new ConflictException('基地负责人角色异常，请先修正负责人角色后再删除基地');
      }
      throw error;
    }
    ...
    await this.operationLogService.logWithContext({
      ...
      userId: operatorId,
      request: context?.request,
      description: `Soft delete base: baseId=${id}`,
      beforeData: purgeResult.beforeSnapshot,
      afterData: {
        softDeleted: true,
        alreadyDeleted: purgeResult.alreadyDeleted,
        counts: purgeResult.counts,
      },
    });

    return { msg: purgeResult.alreadyDeleted ? '基地已处于删除状态' : '基地删除成功' };
  }
  ...
  async getJobsByBase(baseId: number, query: any = {}): Promise<RecruitmentJob[]> {
    ...
    const onlyValid = query.onlyValid === true || query.onlyValid === 'true' || query.onlyValid === '1';
