...
DELIMITER ;;
/*!50003 CREATE*/ /*!50003 TRIGGER `TRG_base_info_validate_owner_before_insert` BEFORE INSERT ON `base_info` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50003 TRIGGER `TRG_base_info_validate_owner_before_update` BEFORE UPDATE ON `base_info` FOR EACH ROW BEGIN
         IF (SELECT role_key FROM sys_user WHERE id = NEW.owner_id) NOT IN ('base_manager', 'boss') THEN
           SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'base owner must be base_manager or boss';
         END IF;
       END */;;
...
CREATE TABLE `daily_signup` (
  ...
  `updated_at` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6),
  PRIMARY KEY (`id`),
  UNIQUE KEY `UQ_daily_signup_user_base_date` (`user_id`,`base_id`,`work_date`),
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50003 TRIGGER `TRG_sys_user_validate_role_before_insert` BEFORE INSERT ON `sys_user` FOR EACH ROW BEGIN
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50003 TRIGGER `TRG_sys_user_validate_role_before_update` BEFORE UPDATE ON `sys_user` FOR EACH ROW BEGIN
         IF NEW.role_key = 'field_manager' AND NEW.assigned_base_id IS NULL THEN
           SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'field_manager must have assigned_base_id';
         END IF;
         IF NEW.role_key <> 'field_manager' AND NEW.assigned_base_id IS NOT NULL THEN
...
/*!50003 SET sql_mode              = 'IGNORE_SPACE,ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50003 TRIGGER `TRG_sys_user_guard_owner_role_before_update` BEFORE UPDATE ON `sys_user` FOR EACH ROW BEGIN
