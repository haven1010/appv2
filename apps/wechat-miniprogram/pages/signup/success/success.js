...
Page({
  ...
  onLoad(options = {}) {
    ...
    this.setData({
      ...
      workerPhone: userInfo.phone || '',
      workerIdCard: userInfo.idCard || '',
      canCancel: Boolean(baseId && workDate),
    });
  },

  onAmountInput(e) {
    this.setData({ totalAmountInput: e.detail.value || '' });
  },
  ...
  async executeCancelSignup() {
    ...
    try {
      const payload = this.data.signupId
        ...
        : { baseId: this.data.baseId, jobId: this.data.jobId, workDate: this.data.workDate };
  ...
  async completeWorklog() {
    if (this.data.completing) return;
    const amount = String(this.data.totalAmountInput || '').trim();
    if (!amount) {
      wx.showToast({ title: '请填写本次工资', icon: 'none' });
      return;
    }
    ...
    try {
      await app.request({
        url: '/worklog/complete',
        method: 'POST',
        data: {
          userId: Number((wx.getStorageSync('userInfo') || {}).id || 0),
          name: this.data.workerName,
          phone: this.data.workerPhone,
          idCard: this.data.workerIdCard,
          baseName: this.data.baseName,
          jobTitle: this.data.jobTitle,
          workStartDate: this.data.workDate,
          workEndDate: formatDateText(new Date().toISOString()),
          totalAmount: Number(amount || 0),
          salaryStatus: 'pending',
          remark: String(this.data.remarkInput || '').trim() || '工人手动结束本次务工',
        },
      });
      wx.showToast({ title: '务工结束已归档', icon: 'success' });
      setTimeout(() => {
