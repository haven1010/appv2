...
function normalizeApplicants(list, targetJobId) {
  return toArray(list)
    .filter((item) => !targetJobId || Number(item.jobId || item.job?.id || 0) === targetJobId)
    .map((item, index) => ({
      id: Number(item.id || 0),
...
Page({
  ...
  async loadApplicants() {
    ...
    try {
      const res = await app.request({
        url: `/base/${this.data.baseId}/applications`,
        method: 'GET',
      });

      const targetJobId = Number(this.data.jobId || 0);
      const applicants = normalizeApplicants(res, targetJobId);
      ...
      success: async (res) => {
        ...
        try {
          await app.request({
            url: `/base/applications/${id}/review`,
            method: 'PATCH',
            data: { status: 2, rejectReason: '当前岗位名额有限，请等待下一轮安排' },
          });
